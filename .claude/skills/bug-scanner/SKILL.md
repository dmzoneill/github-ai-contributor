---
name: bug-scanner
description: Scan upstream repo codebases for critical bugs, open bug report issues, implement fixes, and create PRs referencing those issues. Proactively finds defects by reading source code. Max 2 bug fix PRs per repo, 5 repos per iteration.
...
- **Max 3 total open PRs per upstream repo** (shared limit with coding agent) — skip if at limit
- Count how many of our open PRs are bug fixes from this agent (check `bug_fixes` state) — **max 2 bug fix PRs per upstream repo**
- Collect all open issue titles and bodies — used later to check if a bug is already reported

### Select 5 Repos

Sort repos by current open PR count (ascending) for balanced distribution. Select the first 5 repos that are below limits. Stop after 5 repos.
...
- **Max 2 bug fix PRs per upstream repo** — check before creating
- **Max 3 total open PRs per upstream repo** (shared with coding agent)
- **Scan only 5 repos per iteration** — then stop
